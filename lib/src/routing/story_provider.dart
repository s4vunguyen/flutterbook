...
class StoryProvider extends ChangeNotifier implements ControlsInterface {
  ...
  factory StoryProvider.fromPath(String? path, List<ComponentState> stories) {
    final storyPath = path?.replaceFirst('/stories?p=', '') ?? '';
  ...
  @override
  bool boolean({required String label, bool initial = false, String description = kDefaultNoDescMessage}) =>
      addControl(BoolControl(label, initial, initial, description));

  @override
  String text({required String label, String initial = '', String description = kDefaultNoDescMessage}) =>
      addControl(StringControl(label, initial, initial, description));

  T addControl<T>(Control<T> value) => (_controls.putIfAbsent(value.label, () => value) as Control<T>).value;
